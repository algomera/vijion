                ...
                <div class="flex items-center w-full max-w-sm">
                    <div class="relative flex w-full">
                        <input wire:model.debounce.500ms="search" type="text" name="search"
                               ...
                               placeholder="{{ __('general.Cerca su VIJI-STORE..') }}">
                        ...
                        @if($search_results)
                            <div
                                class="max-h-72 overflow-y-scroll absolute top-12 z-10 mt-2 w-full origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                                ...
                                <div class="py-1" role="none">
                                    ...
                                    @empty
                                        <p class="text-gray-700 block px-4 py-2 text-sm">{{ __('general.Nessun risultato trovato') }}</p>
                                    @endforelse
                                </div>
                            </div>
                        @endif
                    ...
                    <div x-data="{ open: false }" x-on:click="open = true"
                        ...
                        <div class="flex items-center space-x-2 h-full hover:text-brand hover:cursor-pointer"
                            ...
                            <span>{{ __('general.Categorie') }}</span>
                        ...
                        <div x-cloak x-show="open"
                            ...
                            <div class="flex p-4 gap-x-20">
                                <div class="w-1/2 space-y-1.5 divide-y">
                                    <h3 class="text-gray-400 text-xs uppercase font-light">{{ __('general.Categorie') }}</h3>
                                    <div class="space-y-4 pt-5">
                                        @foreach($categories as $category)
                                            <div
                                                class="group relative flex items-center gap-x-6 rounded-lg text-sm leading-6">
                                                <div class="flex-auto">
                                            ...
                                            <div class="flex-auto">
                                                <a href="{{ route('categories') }}"
                                                   class="flex items-center space-x-3 text-brand hover:text-brand">
                                                    <span>{{ __('general.Mostra tutte le categorie') }}</span>
                                ...
                                <div class="w-1/2 space-y-1.5 divide-y">
                                    <h3 class="text-gray-400 text-xs uppercase font-light">{{ __('general.Brand') }}</h3>
                                    <div class="space-y-4 pt-5">
                                        ...
                                        <div
                                            ...
                                            <div class="flex-auto">
                                                <a href="{{ route('brands') }}"
                                                   class="flex items-center space-x-3 text-brand hover:text-brand">
                                                    <span>{{ __('general.Mostra tutti i brand') }}</span>
                                                    <x-heroicon-o-arrow-long-right
                                                        class="w-5 h-5 transform group-hover:translate-x-1 transition"/>
                                                </a>
                                            </div>
            ...
            <a href="{{ route('cart') }}" class="relative mr-4 text-brand hover:text-brand-light">
                @auth
                    @if(auth()->user()->cart_codes->count())
                        <div
                            class="absolute flex items-center justify-center ring-2 ring-white w-4 h-4 text-white text-[11px] font-semibold bg-red-500 rounded-full -right-1.5 -top-1.5">{{ auth()->user()->cart_codes->count() }}</div>
                    @endif
                @endauth
        ...
        <div class="hidden lg:flex lg:items-center lg:space-x-3">
            @auth
                <div class="flex items-center space-x-3 border border-gray-200 rounded-full p-1">
                    ...
                    <div class="bg-brand rounded-full text-sm text-white py-2 px-4">
                        <p class="font-bold">
                            {{ auth()->user()->coins }} <span
                                class="hidden xl:inline-block text-xs font-semibold">{{ __('general.VIJI-COINS') }}</span>
                        </p>
                    </div>
                    @endunlessrole
                    <div class="flex items-center space-x-2 ml-1 text-sm text-gray-500">
                        <p class="font-semibold">{{ __('general.saluto', ['name' => auth()->user()->first_name]) }}</p>
            ...
            @else
                <button wire:click="auth0login"
                    ...
                    <span class="text-sm">{{ __('general.Accedi') }}</span>
            ...
            <div class="flex items-center space-x-2 text-sm text-gray-500">
                <x-dropdown x-data align="right" width="48">
                    <x-slot name="trigger">
                        <p class="font-semibold hover:text-brand cursor-pointer">{{ config('languages.'.app()->getLocale()) }}</p>
                    </x-slot>
                    <x-slot name="content">
                        @foreach(config('languages') as $code => $language)
                            @if(app()->getLocale() !== $code)
                                <x-dropdown-link x-on:click="Livewire.emit('changeLanguage', '{{ $code }}')">
            ...
            <a href="https://vijion.tv/" target="_blank"
               class="rounded-md bg-white mx-2 py-2.5 px-4 text-xs font-bold text-brand-orange uppercase ring-1 ring-inset ring-brand-orange hover:bg-brand-orange hover:text-white">{{ __('general.Vai su Vijion') }}</a>
        </div>
    </nav>
            ...
            <div class="mt-6 flow-root">
                ...
                <div class="divide-y divide-gray-500/10">
                    <div class="space-y-2 py-6">
                        <div
                            class="group relative flex items-center py-2 gap-x-6 rounded-lg text-base font-semibold leading-7">
                            <div class="flex-auto">
                                <a href="{{ route('categories') }}"
                    ...
                    <div class="py-6 text-center">
                        @auth
                            <div class="inline-flex items-center space-x-8 border border-gray-200 rounded-full p-1">
                                ...
                                <div class="bg-brand rounded-full text-sm text-white py-2 px-4">
                                    <p class="font-bold">
                                        {{ auth()->user()->coins }} <span
                                            class="inline-block text-xs font-semibold">{{ __('general.VIJI-COINS') }}</span>
                                    </p>
                                </div>
                                @endunlessrole
                                <div class="flex items-center space-x-2 ml-1 text-sm text-gray-500">
                        ...
                        @else
                            <button wire:click="auth0login"
                                ...
                                <span class="text-center">{{ __('general.Accedi') }}</span>
                                <x-heroicon-o-user-circle class="w-6 h-6"/>
                            </button>
                        @endauth
                    </div>
