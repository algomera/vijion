        ...
        <div>
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm leading-6 text-gray-600">Modifica il Coupon di "{{ $coupon->brand->name }}
                        "</p>
                </div>
                @if($coupon->active)
                    <x-primary-button wire:click="$toggle('coupon.active')"
                                      class="!py-2 !px-2 bg-red-400 text-red-800 hover:bg-red-300">Disattiva
                    </x-primary-button>
                @else
                    <x-primary-button wire:click="$toggle('coupon.active')"
                                      class="!py-2 !px-2 bg-green-400 text-green-800 hover:bg-green-300">Attiva
                    </x-primary-button>
                @endif
            </div>
            ...
            <div class="mt-8 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                <div class="col-span-full sm:col-span-3">
                    <x-input wire:model.debounce.500ms="coupon.amount" type="number" name="coupon.amount"
                             label="Valore"></x-input>
                </div>
                <div class="col-span-full sm:col-span-3">
                    <x-input wire:model.debounce.500ms="coupon.coins" type="number" step="1" name="coupon.coins"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start gap-4 mt-3">
                        <div class="flex-1">
                            ...
                            <x-coupon-card-highlight-preview :brand="$coupon->brand"
                                                             ...
                                                             :text_color="$coupon->text_color"
                                                             :amount="$coupon->amount ?? 0"
                                                             :coins="$coupon->coins ?? 0"
                                                             :type="$coupon->type"
                                                             :expires_date="$coupon->expires_date"/>
                        </div>
                        <div class="flex-1">
                            <x-input-label class="mb-1">Card</x-input-label>
                            <div class="aspect-video">
                                <x-coupon-card-preview :brand="$coupon->brand" :amount="$coupon->amount ?? 0"
                                                       :coins="$coupon->coins ?? 0"
                                                       :type="$coupon->type"/>
                            </div>
                        </div>
