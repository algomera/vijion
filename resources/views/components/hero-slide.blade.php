...
<div class="relative isolate overflow-hidden bg-gray-900 py-16">
	<img src="{{ url($slide->background_url) }}" alt="" class="absolute inset-0 -z-10 h-full w-full object-cover">
	...
	<div class="flex flex-col items-center justify-center drop-shadow">
		<p class="text-2xl text-brand">{{ $slide->small_centered_text }}</p>
		<h2 class="text-9xl font-narrow font-bold text-brand">{{ $slide->big_centered_text }}</h2>
		<div class="flex items-center space-x-2 mt-3 px-3 py-1 bg-white/70 border border-white text-white rounded">
			...
			<span class="text-xl font-bold">{{ $slide->coins_centered_text }}</span>
		</div>
	</div>
</div>
